...
# Steps 
0. If using docker container, first build a container using the provided Dockerfile. All subsequent code will be run in this container. You can run 00_run_pipeline.sh to run the following steps
1. Trim reads and QC sequencing with 01_fastqc.sh
2. Pseudoalign and quantify reads with 02_kallisto.sh
...
4. Differential gene expression and plot generation. Run 03_deseq2.Rmd
...
│
├── scripts/
|   ├── 00_run_pipeline.sh           #runs the subsequent pipeline 
│   ├── 01_fastqc.sh                 # fastp trimming and fastqc
│   ├── 02_kallisto.sh               # kallisto quant
│   ├── 03_deseq2.Rmd                # DESeq2 analysis
│   └── aggregate_mapping_reads.py   # aggregate read mapping stats
│
├── data/
